...
#ifndef include_pot_util_h
#define include_pot_util_h
 ...
 *
 * Step 1: Initialize this library by calling pot_util_init()
...
static inline u16
pot_profile_get_id (pot_profile * profile)
{
  if (profile)
    {
      return (profile->id);
...
static inline u8
pot_is_decap (pot_profile * p)
{
  return (p->validator == 1);
}

static inline int
pot_profile_set_active (u8 id)
{
...
static inline u8
pot_profile_get_active_id (void)
{
  pot_main_t *sm = &pot_main;
  return (sm->active_profile_id);
}

static inline pot_profile *
...
static inline void
pot_profile_reset_usage_stats (pot_profile * pow)
{
  if (pow)
    {
...
static inline void
pot_profile_incr_usage_stats (pot_profile * pow)
{
  if (pow)
    {
...
int pot_profile_to_str (pot_profile * profile, char *buf, int n);
#endif
