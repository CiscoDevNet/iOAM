...
import org.opendaylight.yang.gen.v1.urn.cisco.params.xml.ns.yang.ioam.nb.system.rev151122.*;
import org.opendaylight.yang.gen.v1.urn.cisco.params.xml.ns.yang.ioam.nb.system.rev151122.nb.sys.profiles.*;
import org.opendaylight.yang.gen.v1.urn.cisco.params.xml.ns.yang.ioam.nb.system.rev151122.nb.sys.profiles.nb.sys.profile.*;
import org.opendaylight.yang.gen.v1.urn.cisco.params.xml.ns.yang.ioam.nb.trace.rev160202.*;
import org.opendaylight.yang.gen.v1.urn.cisco.params.xml.ns.yang.ioam.nb.trace.rev160202.trace.profiles.*;
...
public class SfcProviderRenderedPathAPI {
    ...
    /**
     * This function creates a RIP in the datastore
     * <p>
     * @param SysName sys profile name
     * @return RIP name.
     */
    public static String createRenderedIoamPath(String sysName) {
        String ripName = null;
        RenderedIoamPathBuilder renderedIoamPathBuilder = new RenderedIoamPathBuilder();
        List<NodesBorder> nodesBorderList;
        List<NodesInternal> nodesInternalList;

        List<SbNodesBorder> sbNodesBorderList = new ArrayList<>();
        List<SbNodesInternal> sbNodesInternalList = new ArrayList<>();

        NbSysProfile nbs = null;
        NbSysProfileKey nbsKey = new NbSysProfileKey(sysName);
        InstanceIdentifier<NbSysProfile> nbsIID = InstanceIdentifier.builder(NbSysProfiles.class).child(NbSysProfile.class, nbsKey).build();
        nbs = SfcDataStoreAPI.readTransactionAPI(nbsIID, LogicalDatastoreType.CONFIGURATION);

        if (nbs == null) {
            LOG.error("SFCV:Failed to create Rendered Ioam Path:nbs null");
            return null;
        }

        //long pathId = numCreatedPathIncrementGet();
        long pathId = SfcServicePathId.check_and_allocate_pathid();
        ripName = new String(sysName + "-Path-" + pathId);

        List<IoamProfiles> oamP = nbs.getIoamProfiles();
        if (oamP == null) {
            LOG.error("SFCV:Failed to create Rendered Ioam Path:oamp null");
            return null;
        }

        IoamProfiles prof = oamP.get(0);
        if (prof == null) {
            LOG.error("SFCV:Failed to create Rendered Ioam Path:prof null");
            return null;
        }
        String traceProfName = prof.getTraceProfileName();
        if (traceProfName == null) {
            LOG.error("SFCV:Failed to create Rendered Ioam Path:traceProfName null");
            return null;
        }

        TraceProfileKey traceKey = new TraceProfileKey(traceProfName);
        InstanceIdentifier<TraceProfile> traceIID = InstanceIdentifier.builder(TraceProfiles.class).child(TraceProfile.class, traceKey).build();
        TraceProfile trc = SfcDataStoreAPI.readTransactionAPI(traceIID, LogicalDatastoreType.CONFIGURATION);
        short traceVal = (short)trc.getTraceType().getIntValue();

        Boolean tracePpcEnable = trc.isTracePpcEnable();
        Boolean tracePowEnable = trc.isTracePowEnable();

        //nbs.getNodesBorder();
        nodesBorderList = nbs.getNodesBorder();
        for (NodesBorder nbb: nodesBorderList) {
            SbNodesBorderBuilder sbb = new SbNodesBorderBuilder();
            sbb.setNetconfNode(nbb.getNetconfNode()).setNodeId(nbb.getNodeId()).setIndex(nbb.getIndex());
            sbNodesBorderList.add(sbb.build());
        }

        nodesInternalList = nbs.getNodesInternal();
        for (NodesInternal nbi: nodesInternalList) {
            SbNodesInternalBuilder sbi = new SbNodesInternalBuilder();
            sbi.setNetconfNode(nbi.getNetconfNode()).setNodeId(nbi.getNodeId()).setIndex(nbi.getIndex());
            sbNodesInternalList.add(sbi.build());
        }

        renderedIoamPathBuilder.setName(ripName);
        renderedIoamPathBuilder.setNbSysProfileName(sysName);
        renderedIoamPathBuilder.setSbTraceType(new Short(traceVal));
        renderedIoamPathBuilder.setSbTracePpc(tracePpcEnable);
        renderedIoamPathBuilder.setSbTracePow(tracePowEnable);
        renderedIoamPathBuilder.setSbNodesBorder(sbNodesBorderList);
        renderedIoamPathBuilder.setSbNodesInternal(sbNodesInternalList);

        RenderedIoamPathKey renderedIoamPathKey = new
                RenderedIoamPathKey(renderedIoamPathBuilder.getName());
        InstanceIdentifier<RenderedIoamPath> ripIID;
        ripIID = InstanceIdentifier.builder(RenderedIoamPaths.class)
                .child(RenderedIoamPath.class, renderedIoamPathKey)
                .build();

        RenderedIoamPath renderedIoamPath =
                renderedIoamPathBuilder.build();

        if (SfcDataStoreAPI.writeMergeTransactionAPI(ripIID, renderedIoamPath, LogicalDatastoreType.OPERATIONAL)) {
            return ripName;
        } else {
            LOG.error("SFCV:{}: Failed to create Rendered Ioam Path: {}",
                    Thread.currentThread().getStackTrace()[1], ripName);
        }
    ...
    /**
     * This function deletes a RIP from the datastore
     * <p>
     * @param renderedIoamPathName RIP name
     * @return Nothing.
     */
    public static boolean deleteRenderedIoamPath(String renderedIoamPathName) {
        boolean ret = false;
        printTraceStart(LOG);
        RenderedIoamPathKey  renderedIoamPathKey = new RenderedIoamPathKey(renderedIoamPathName);
        InstanceIdentifier<RenderedIoamPath> ripEntryIID = InstanceIdentifier.builder(RenderedIoamPaths.class)
                .child(RenderedIoamPath.class, renderedIoamPathKey).build();

        if (SfcDataStoreAPI.deleteTransactionAPI(ripEntryIID, LogicalDatastoreType.OPERATIONAL)) {
            ret = true;
        } else {
            LOG.error("SFCV:{}: Failed to delete RIP: {}", Thread.currentThread().getStackTrace()[1],
                    renderedIoamPathName);
        }
        printTraceStop(LOG);
        return ret;
    }
