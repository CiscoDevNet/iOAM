...
public class SfcProviderRpc implements ServiceFunctionService, ServiceFunctionChainService, RenderedServicePathService,
        ServicePathIdService, RenderedIoamPathService {
    ...
    @Override
    public Future<RpcResult<CreateIoamPathOutput>> createIoamPath(final CreateIoamPathInput input) {
        NbSysProfile nbs = null;
        InstanceIdentifier<NbSysProfile> nbsIID;
        String nbSysProfileName = null;
        RpcResult <CreateIoamPathOutput> rpcResult;
        CreateIoamPathOutputBuilder createIoamPathOutputBuilder = new CreateIoamPathOutputBuilder();
        String retRipName = null;

        retRipName = SfcProviderRenderedPathAPI.createRenderedIoamPath(input.getNbSysProfileName());

        if (retRipName != null) {
            createIoamPathOutputBuilder.setName(retRipName);
            rpcResult = RpcResultBuilder.success(createIoamPathOutputBuilder.build()).build();
            return Futures.immediateFuture(rpcResult);
        } else {
            rpcResult = RpcResultBuilder.<CreateIoamPathOutput>failed()
                        .withError(ErrorType.APPLICATION, "Failed to create Rendered path.").build();
            return Futures.immediateFuture(rpcResult);
        }
    }
    ...
    @Override
    public Future<RpcResult<DeleteIoamPathOutput>> deleteIoamPath(DeleteIoamPathInput input) {
        boolean ret;
        RpcResultBuilder<DeleteIoamPathOutput> rpcResultBuilder;

        ret = SfcProviderRenderedPathAPI.deleteRenderedIoamPath(input.getName());
        DeleteIoamPathOutputBuilder deleteIoamPathOutputBuilder = new DeleteIoamPathOutputBuilder();
        deleteIoamPathOutputBuilder.setResult(ret);
        if (ret) {
            rpcResultBuilder =
                    RpcResultBuilder.success(deleteIoamPathOutputBuilder.build());
        } else {
            String message = "Error Deleting Ioam Path: " + input.getName();
            rpcResultBuilder =
                    RpcResultBuilder.<DeleteIoamPathOutput>failed()
                            .withError(ErrorType.APPLICATION, message);
        }
