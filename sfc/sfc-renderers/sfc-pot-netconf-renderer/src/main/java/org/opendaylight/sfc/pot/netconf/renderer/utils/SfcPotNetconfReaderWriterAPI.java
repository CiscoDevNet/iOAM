...
public class SfcPotNetconfReaderWriterAPI {
    ...
    public static NetconfNode readNode(NodeId nodeId) {
        try {
            Node topoNode =
                SfcDataStoreAPI.readTransactionAPI(NETCONF_TOPO_IID.child(Node.class, new NodeKey(nodeId)),
    ...
    public static <T extends DataObject> boolean put(NodeId nodeId, LogicalDatastoreType logicalDatastoreType,
        ...
        if (mountPointService == null) {
            mountPointService = SfcPotNetconfDataProvider.getNetconfDataProvider().getMountService();
        }
        try {
            Optional<MountPoint> mountPointOptional =
                 mountPointService.getMountPoint(NETCONF_TOPO_IID.child(Node.class, new NodeKey(nodeId)));
